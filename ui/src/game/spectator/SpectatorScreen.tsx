          ...
          {categories.map(({ name, questions }, idx) => {
            return (
              <div key={idx} className="flex flex-col min-w-48 grow flex-1 mx-2 gap-4">
                ...
                {questions.map((q) => {
                  return q.answered ? (
                    <div key={q.questionId} className="bg-gray-200 min-h-16 flex-1 rounded-lg"></div>
                  ) : (
                    <div
                      key={q.questionId}
                      className="flex items-center justify-center bg-orange-400 text-white rounded-lg min-h-16 flex-1 p-0"
                    >
